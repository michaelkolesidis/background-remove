// Place a newBackground.png file in data folder
...
void setup() {
  ...
  newBackground = loadImage("newBackground.png");
}
  ...
  for (int x = 0; x < video.width; x ++ ) {
    for (int y = 0; y < video.height; y ++ ) {
      ...
      if (diff > threshold) {
        ...
        pixels[loc] = newBackground.pixels[loc]; // We could choose to replace the background pixels with something other than the color green!
      }
    }
  }
  updatePixels();
  threshold = map(mouseX, 0, width, 10, 30);
}
